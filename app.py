...
# DraftKings lineup slots (for parsing the Lineup column)
POS_SLOTS = ["PG", "SG", "SF", "PF", "C", "G", "F", "UTIL"]

# Ownership bucket thresholds (must match builder.py)
...
def get_structure_rule(
    ...
    if ct == "SE":
        # Separate "small SE" vs "big SE" by field size
...
def classify_user_style(
    ...
    """
    Compare a user's average bucket counts to each StructureRule
    ...
    for rule in styles:
        mega_mid = (rule.mega_range[0] + rule.mega_range[1]) / 2.0
        chalk_mid = (rule.chalk_range[0] + rule.chalk_range[1]) / 2.0
        mid_mid = (rule.mid_range[0] + rule.mid_range[1]) / 2.0
        punt_mid = (rule.punt_range[0] + rule.punt_range[1]) / 2.0
...
def extract_username(entry_name: str) -> str:
    """
    DK EntryName often looks like 'youdacao (5/150)'.
    We just want 'youdacao'.
    """
...
def parse_lineup_string(lineup: str):
    """
    Parse the DK "Lineup" string into list of dicts:
      [{ "pos_slot": "PG", "player_name": "Luka Doncic" }, ...]
    """
    if pd.isna(lineup):
        return []
...
def build_long_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Turn the DK standings CSV into a long format:
    ...
    if missing:
        st.error("Contest CSV is missing required columns: {}".format(missing))
        st.stop()
...
def add_field_ownership(long_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Compute field ownership for each player in this contest:
      field_own = (# of lineups with player) / (# of lineups in contest)
    """
    ...
    own["field_own"] = own["lineups_with_player"] / float(total_entries)
...
def build_lineup_features(long_df: pd.DataFrame) -> pd.DataFrame:
    """
    Build lineup-level summary:
    - avg_own, sum_own, max_own, min_own
    - counts of mega / chalk / mid / punts
    """
...
def get_lineup_detail(long_df: pd.DataFrame, entry_id) -> pd.DataFrame:
    """Return the players + ownership for a single lineup."""
...
def load_slate_players_from_upload(uploaded_file) -> pd.DataFrame:
    ...
    missing = [c for c in required if c not in df.columns]
    if missing:
        st.error("Slate / projections CSV is missing required columns: {}".format(missing))
        st.stop()

    # Bucket each player by ownership
...
st.markdown(
    """
Upload:
1. A **DraftKings results CSV** (standings export) to analyze what won.  
2. A **slate + projections CSV** (player pool) to build sample lineups
   that fit contest-specific ownership structures.
...
with tab_overview:
    ...
    if lineup_summary is None:
        ...
        st.sidebar.markdown("**Top cutoff rank:** {} / {}".format(top_cut_rank, total_entries))
        st.sidebar.markdown("---")
...
"""
        ...
        col3.metric("Median points (all)", "{:.2f}".format(lineup_summary["points"].median()))
        col4.metric("Median avg ownership (all)", "{:.2%}".format(lineup_summary["avg_own"].median()))
        ...
        renamed = lineup_summary[simple_cols].rename(
            columns={
                ...
                "is_top": "Winner? (Top {}%)".format(top_pct),
            }
        )
...
"""
    ...
    if slate_file is None:
        ...
        player_view["own_proj"] = player_view["own_proj"].map(lambda x: "{:.1%}".format(x))
        ...
        if st.button("Build sample lineup"):
            if "Showdown" in slate_mode:
                ...
                st.write("Using structure profile: **{}** (Classic, 8 players)".format(template.contest_label))
            ...
            if lineup is None or lineup.empty:
                ...
                show["own_proj"] = show["own_proj"].map(lambda x: "{:.1%}".format(x))
                ...
                st.write("**Total salary:** {} / 50000".format(total_salary))
                st.write("**Total projection:** {:.2f} fpts".format(total_proj))
...
with tab_patterns:
    ...
    if lineup_summary is None or top_df is None or rest_df is None:
        ...
        top_stats = summarize(top_df, "Winners (Top {}%)".format(top_pct))
        ...
        table["Mean Points"] = table["Mean Points"].map(lambda x: "{:.2f}".format(x))
        table["Median Points"] = table["Median Points"].map(lambda x: "{:.2f}".format(x))
...
with tab_players:
    ...
    if long_df is None or top_df is None or rest_df is None:
        ...
        g["top_own"] = g["lineups_top"] / float(max(1, n_top))
        g["rest_own"] = g["lineups_rest"] / float(max(1, n_rest))
        ...
        for col in ["field_own", "top_own", "rest_own", "top_minus_rest"]:
            display_df[col] = display_df[col].map(lambda x: "{:.1%}".format(x))
...
with tab_users:
    ...
    if lineup_summary is None or long_df is None:
        ...
        st.markdown("**{}** – {} lineups".format(selected_user, len(user_group)))
        ...
        st.markdown(
            """
On average, **{}** built lineups like this:

- Avg points per lineup: **{:.2f}**
- Avg player ownership: **{:.1%}**

Per lineup (on average):

- Mega chalk (≥40%): **{:.2f}** players
- Chalk (30–39%): **{:.2f}** players
- Mid (10–29%): **{:.2f}** players
- Punts (<10%): **{:.2f}** players
""".format(
                selected_user,
                user_stats["avg_points"],
                user_stats["avg_avg_own"],
                user_stats["avg_mega"],
                user_stats["avg_chalk"],
                user_stats["avg_mid"],
                user_stats["avg_punt"],
            )
        )
...
> **{}**-style lineups (distance score {:.2f})
...
- Avg player ownership (winners, top {}%): **{:.1%}**
- Avg player ownership (everyone else): **{:.1%}**
""".format(
                top_pct, top_mean_avg_own, rest_mean_avg_own
            )
        )
...
**Winners (top {}%):**
- Mega chalk (≥40%): **{:.2f}** per lineup
- Punts (<10%): **{:.2f}** per lineup
