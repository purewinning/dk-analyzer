...
def tab_lineup_builder(slate_df, template):
    ...
    if st.button("Generate Optimal Lineup"):
        ...
        with st.spinner(f'Calculating {template.contest_label} optimal lineup...'):
            # Consolidating call to single line to prevent copy-paste errors
            optimal_lineup_df = build_optimal_lineup(slate_df=slate_df, template=template, bucket_slack=1)
