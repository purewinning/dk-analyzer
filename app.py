...
# Entry fee tiers – used for hints
ENTRY_FEE_TIERS = {
...
def display_lineup_results(
    ...
    # Order: best strategy fit, then projection
...
with st.container():
    ...
    with c3:
        # size band is just a label for vibes
    ...
    with c4:
        total_entries = st.number_input(
            ...
            value=min(583, entry_fee_info["max_field"]),
            step=50,
        )
    ...
    # map contest to internal code & profile
    tournament_map = {
    ...
    with col_summary1:
        st.metric("Field Size", f"{total_entries:,}")
    with col_summary2:
...
with st.container():
    ...
    if slate_df.empty:
        ...
        # Player editor with Lock / Exclude + edge columns
        ...
        if run_btn:
            ...
            if conflict:
                ...
                if not raw_lineups:
                    ...
                    # If auto_match, we'll score vs profile and keep top N
                    if auto_match:
                        scored = []
